...
BEGIN
	...
	VALUES	(@group, @discipline, @teacher, @date, @time, IIF(@data < GETDATE(), 1, 0));
	...
	INSERT	Schedule
			([group], discipline, teacher, [date], [time], spent)
	VALUES	(@group, @discipline, @teacher, @date, DATEADD(MINUTE, 95, @time), IIF(@data < GETDATE(), 1, 0));
	SET @lesson_number = @lesson_number + 1;
	-------------------------------------------
	PRINT(@lesson_number + 1);
	...
	INSERT	Schedule
			([group], discipline, teacher, [date], [time], spent)
	VALUES	(@group, @discipline, @teacher, @date, DATEADD(MINUTE, 190, @time), IIF(@data < GETDATE(), 1, 0));
	SET @lesson_number = @lesson_number + 1;
	PRINT('===================================');
